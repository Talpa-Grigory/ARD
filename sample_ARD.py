...
from models2 import DiT_models
...
def main(args):
    ...
    ckpt_path = args.ckpt_path  # or f"DiT-XL-2-{args.image_size}x{args.image_size}.pt"
    state_dict = find_model(ckpt_path)["ema"]
    model.load_state_dict(state_dict)
    model.eval()  # important!
    vae = AutoencoderKL.from_pretrained(f"stabilityai/sd-vae-ft-{args.vae}").to(device)

    # Create folder to save samples:
    folder_name = f"samples"
    ...
    for i in pbar:
        ...
        t = torch.ones((z.shape[0]), device=device) * 999.0
        pred_z15, pred_z10, pred_z5, pred_z0 = model.infer(z, t, args.stack, **model_kwargs)
        samples = vae.decode(pred_z0 / 0.18215).sample
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample-dir", type=str, default="sample_ddp")
